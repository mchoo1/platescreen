...
export const RESEARCH_QUEUE = [
  ...
  },
  {
    ...
    priority: "high",
    status: "pending",
    notes: "2026-08-22: Kopitiam is no longer a Brand — it was a single 'brand' row standing in for 48 different physical food-court buildings, each containing many unrelated stalls with completely different food, which never had a real menu and never rendered (buildScreenerRows joins off MenuItems). Restructured per the Operator design in types/db.ts: Kopitiam stays in operators.ts as the operating company; its 48 real SFA-sourced building addresses are preserved in reference/data/food-court-venues.json (operatorId: 'kopitiam') as research anchors. Real work item: visit each venue, identify NAMED concessions inside it (e.g. a Yakun or Ya Kun Kaya Toast stand, a specific fried rice stall with its own signage), and add each as its own Brand with operatorId: 'kopitiam' + its own Premises + MenuItems — exactly the pattern already done correctly for Hawkers' Street's 27 real stalls. Do not re-add Kopitiam itself as a Brand. UPDATE 2026-08-22 (later same day): visited kopitiam.biz per user request. kopitiam.biz/our-outlets/ is a JS/OneMap map widget with no server-rendered outlet list and no discoverable REST/AJAX endpoint (checked network requests + inline scripts + wp-json — nothing) — could not bulk-extract. FairPrice Group's own site (fairpricegroup.com.sg/our-group/food-services) does list Kopitiam's named in-house F&B brands directly, though: Bagus / Bagus Food Hall (halal concept, 11 locations, flagship at Northpoint City), Kopitiam Corner (coffeeshop format), and named concessions Ah Bowl Den, Belly Belly Good Cai Fan, Chomp!, Confirm+Chop Western Grill, Flint Specialty Grill, Flint Specialty Grill & Bar, Heavenly Wang, Kokoro Izakaya, Kokoro Kiosuku, Sedap Kitchen, Xiang Chi Mian — none of these appeared as filter categories on the outlet-finder page, so their specific addresses need per-brand digging (likely each has its own page/social presence), not yet done. Also keeping Lau Pa Sat and Food Folks @ Lau Pa Sat in mind — FairPrice Group's page confirms Kopitiam Investment Pte Ltd operates Lau Pa Sat + 16 hawker centres, separate from the food-court format."
  },
  {
    id: "koufu",
    name: "Koufu",
