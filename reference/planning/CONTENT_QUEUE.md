...
**Renamed 2026-08-30** from `archive/growth-content-ideas.md` (same content,
same purpose) — a rotating leaderboard/post theme queue intended for a
content-drafting scheduled task. Each theme is computed live from
`src/lib/brands.ts` / `premises.ts` / `menuItems.ts` / `groceryProducts.ts`
at post-draft time — never hardcode numbers here, this file only tracks
WHICH theme to cover and WHEN it was last used. See `GROWTH_STRATEGY.md` for
the overall strategy this supports.

**Note:** the task that originally consumed this file
(`platescreen-post-copilot`) is disabled and being redone from scratch — see
`ROADMAP.md` priority 1. This queue's themes are still reasonable starting
material for whatever replaces it.
...
- Pick the oldest `pending` theme (array order = priority), or the
  least-recently-`used` theme if all are used, so themes rotate rather than
  repeat back-to-back.
- Never post two themes to the same subreddit within a rolling 14-day
  window — check the digest history for this.
- If a theme's `computation` can't currently produce at least 8-10 solid
  rows from real, non-fabricated data, skip it and try the next theme in
  this run rather than posting a thin list — note the skip and leave its
  status as `pending`.
- After a theme is actually posted (confirmed by a human, not by the
  drafting task itself, since posting is always human-gated), flip its
  `status` to `used` and set `lastUsed`/`subreddit`.
